...
// const requestFromExternal = require('superagent');
// const xml2jsParser = require('superagent-xml2jsparser');
//const faketoe = ('faketoe');
...
.get('/search/tv/:id', (req,res,next) => {
  tvdb.getSeriesByName(req.params.id)
  .then( result => {
    res.send(result);
  })
  .catch( err => { 
    console.log('Error searching TVDB for series name: req.body.name=' + req.body.name);
    console.log('Error returned:', err);
    next(err);
  });
})

// .get('/search/movie/:id', (req,res,next) => {
//   tvdb.getSeriesByName(req.params.id)
//   .then( result => {
//     res.send(result);
//   })
...
.get('/search/book/:id', (req,res,next) => {
